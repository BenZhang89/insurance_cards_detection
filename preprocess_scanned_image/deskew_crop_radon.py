...
def process_image(path, out_path):
    ...
    edges = cv2.Canny(np.asarray(im), 150,200)  #np.asarray(im)
    ...
    maxed_rows = rank_filter(edges, -4, size=(1, 4))
    maxed_cols = rank_filter(edges, -4, size=(16, 1))
    ...
    pil_image = orig_im.crop(crop)



    opencvImage = cv2.cvtColor(np.asarray(pil_image), cv2.COLOR_RGB2BGR) 
    h, w, _ = opencvImage.shape 
    if h/w>0.5 and h/w<0.75:   
        #find the skewed angle and deskew it
        I = np.asarray(pil_image.convert('L'))
        ...
        rotated = ndimage.rotate(opencvImage, angle, reshape=False, cval=255)
...
if __name__ == '__main__':
    if len(sys.argv) == 2 and '*' in sys.argv[1]:
        ...
        files = sys.argv[1]
#files is the input file folder name
    files = os.path.join(os.getcwd(), files)
    if not os.path.exists(files+'/cropped'):
        os.makedirs(files+'/cropped')
    outfiles = os.path.join(files, 'cropped')

    for path in os.listdir(files):
        out_path = path.replace('.jpeg', '.crop.png')
        out_path = os.path.join(outfiles, out_path)
        path = os.path.join(files, path)
        # out_path = path.replace('.jpeg', '.crop.png') # 没必要
